...
# of the polarization, which is is uncommon. In most cases, the interaction of
# with the pump leads to an frequency shift, either by promoting the molecule to
# an electronically excited state (visible pump) or to an higher vibrational
# state (IR pump). In our toy model, this can be modeled by switching the frequency
# at 1 ps. Here we assume no phase change.
...
yt = np.exp(-ts / T2) * np.cos(om2*ts*2*np.pi - dphi*2*np.pi)
...
# jumps are exaggerated for common cases.
