...
try:
    from base_funcs.base_functions_cl import (_fold_exp,
                                              _coh_gaussian,
                                              _fold_exp_and_coh)
    print("pyopencl found, using cl-basefunctions")
except ImportError:    
    try:
        from base_funcs.base_functions_numba import (_fold_exp,
                                                     _fold_exp_and_coh,
                                                     _coh_gaussian)
        print("pyopencl not found, using numba-basefunctions")
    except ImportError:
        from base_funcs.base_functions_np import(_fold_exp,
                                                 _fold_exp_and_coh,
                                                 _coh_gaussian)
